...
let li = document.querySelector('.promo__interactive-item')
...
function reload (arr) {
    ...
    for(item of movies) {
        ...
        ul.append(item_list)
        item_list.append(num, title, btn_remove)
        btn_remove.append(img_remove)

        num.classList.add('promo__interactove-item')
        title.classList.add('promo__interactove-item')
        btn_remove.classList.add('btn_remove')
        
        num.innerHTML = item.ID
        img_remove.src = './img/remove.png'
        img_remove.setAttribute('width', '20')
